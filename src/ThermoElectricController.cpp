...
float ThermoElectricController::get_Temperature(int channel)
{
  ...
  // read the analog voltage
  int adcCounts = 0;
  // //TEST_SERIAL.print("Getting Temperature from pin ");//TEST_SERIAL.println( thermistorPin );
  for (int i = 0; i < 16; i++)
  {
  ...
  // //TEST_SERIAL.printf("RawTemperature: %f\n", temperature);
  ...
  if (calibrated)
  {
    temperature = (((temperature - therm[channel].getRaw_low()) * (ref_High - ref_Low)) /
                   ...
                   ref_Low);
    //TEST_SERIAL.printf("CalTemperature: %f\n", temperature);
  }
  return temperature;
}
...
{
  //TEST_SERIAL.printf("Set temp is %0.2f, calibration begun.\n", ref_temp);
  ...
  {
    eeAddr = 1;
    //TEST_SERIAL.println("Cal data 1 INW");
    ref_Low = ref_temp;
  ...
  {
    //TEST_SERIAL.println("Cal data 2 INW");
    ref_High = ref_temp;
    ...
    EEPROM.write(0, 0x01);
    calibrated = true;
    //TEST_SERIAL.println("Calibration complete.");
  }
  return true;
}
